...
export async function makePayment(amount) {
    try {
		...
		const allowanceAmount = await usdtContract.methods.allowance(sender, contract.options.address).call();
        console.log("Allowance amount:", allowanceAmount);

		if (parseInt(allowanceAmount) >= parseInt(amountToSend)) {
		...
		// const approve =await approveAllowance(amountToSend)
        ...
        // Now, call the receiveBUSD function
		console.log(res);
        return true;
    } catch (error) {
